...
│       ├── interview.js    # Interview-specific logic
│       ├── main.js         # Main application logic
│       └── style.css       # Application styling
├── src/                    # TypeScript source files
│   ├── client.ts           # AWS Bedrock client implementation
│   ├── server.ts           # Express server implementation
│   ├── index-cli.ts        # Command-line interface for audio processing
│   ├── consts.ts           # Application constants
│   └── types.ts            # TypeScript type definitions
├── input-audio-example/    # Sample audio files for CLI testing
...
Process pre-recorded audio files without the web interface:
...
This will:
- Read audio from `./input-audio-example/japan16k.raw` (16kHz, mono, s16le format)
- Process it through Amazon Nova Sonic
- Save outputs to `./output/` directory:
  - `output-audio.raw` - Audio response from Nova Sonic
  - `transcription.txt` - Transcribed conversation
  - `analysis.txt` - Any tool analysis results

To convert the output audio to WAV format:
```bash
ffmpeg -f s16le -ar 24000 -ac 1 -i output/output-audio.raw output/output-audio.wav
```
